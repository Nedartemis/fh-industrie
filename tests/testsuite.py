...
def _list_to_dict_occurences(lst: List) -> Dict:
    return dict(Counter(lst).items())


def _wrapper_test_good(runnable: TYPE_RUNNABLE) -> None:
    logger.reset_logs()
    runnable()
    assert len(logger.get_logs_label()) == 0


def _wrapper_test_logs(
    ...
    occs_actual = _list_to_dict_occurences([ll.__class__ for ll in logs_label])
    occs_expected = _list_to_dict_occurences(expected_log_label_class)
    assert occs_actual == occs_expected


def _wrapper_try(runnable: TYPE_RUNNABLE, expected_log_label_class) -> None:
...
def test_read_source_page_raise(
    ...
    _wrapper_try(lambda: _read_sources(filename), expected_log_label_class)
...
def test_read_info_page_raise(
    ...
    _wrapper_try(lambda: _read_infos(filename, "wrong"), expected_log_label_class)
...
def _read_infos_and_preprocess(filename: str) -> Dict[str, InfoExtractionDatas]:
    _, path_config_file = _from_folder_name_and_filename("info_page/good", filename)
...
@pytest.mark.parametrize(
    ...
    [
        (filename,)
        for filename in os.listdir(PATH_TEST_DOCS_TESTSUITE / "info_page/good")
    ],
)
def test_read_info_page_preprocess_all(filename):
    def f():
        _read_infos_and_preprocess(filename[:-5])
        logger.filter_logs_level(WARNING)

    _wrapper_test_logs(f, [])
...
@pytest.mark.parametrize(
    ["filename", "expected_infos"],
    [
        ("values_one_ind_info", iv(inds={"n1": "t1"})),
        ("values_one_ind_info_without_source", iv(inds={"n1": "t1"})),
        ("values_one_ind_info_filter_one_without_value", iv(inds={"n1": "t1"})),
        ("values_one_lst_info", iv(lists={"n1": [{"s1": "t1"}]})),
        ("values_two_lst_info", iv(lists={"n1": [{"s1": "t1"}], "n2": [{"s1": "t2"}]})),
        (
            "values_one_lst_two_sub",
            iv(lists={"n1": [{"s1": "t1", "s2": "t2"}]}),
        ),
        (
            "values_one_lst_two_elements",
            iv(lists={"n1": [{"s1": "t1"}, {"s1": "t2"}]}),
        ),
        ("values_lst_not_eclated", iv()),
        ("values_lst_eclated_without_values", iv()),
        (
            "values_big",
            iv(
                inds={"n2": "t6", "n6": "t8"},
...
def test_read_config_file_wrong(
    ...
    _wrapper_test_good(runnable=lambda: _read_config_file(filename, "good"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [ExtensionFileNotSupported, EmptyInfoExcel],
            {},
            DEFAULT_INFOS,
        ),
        (
...
def test_read_config_file_raise():
    _wrapper_try(lambda: _read_config_file("not_existing", "wrong"), ExcelNotExisting)
...
def test_fill_config_file_raise(filename: str, expected_log_label_class):
    _wrapper_try(
        lambda: _fill_config_file(filename, "wrong", iv()),
        expected_log_label_class,
    )
